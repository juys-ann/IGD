 ...
 * writing area. Elements are positioned absolutely within a relative wrapper.
 * Drag is handled via window mousemove/mouseup so it works outside element bounds.
 */
import { useRef, useEffect } from 'react'

export default function ElementLayer({ elements, onChange, children }) {
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      onChange((prev) => prev.map((el) => el.id === id ? { ...el, x: Math.max(0, origX + dx), y: Math.max(0, origY + dy) } : el))
    }
        ...
        if (el.type === 'photo') return (
          <div key={el.id} className="group" style={{ ...pos, width: el.width ?? 200, cursor: 'grab', userSelect: 'none' }}>
            <div style={{ position: 'relative' }} onMouseDown={(e) => startDrag(e, el)}>
              <img src={el.src} alt="attached" style={{ width: '100%', display: 'block', borderRadius: 6, boxShadow: '0 4px 16px rgba(0,0,0,0.22)' }} draggable={false} />
              {/* Drag hint */}
        ...
        if (el.type === 'sticky') return (
          <div key={el.id} className="group"
            ...
            <textarea
              ...
              onMouseDown={(e) => e.stopPropagation()} // don't drag when clicking textarea
            />
          </div>
        )
